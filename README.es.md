...
Construido en una semana por un equipo de dos personas.

## Capturas

### Dashboard

![Vista del dashboard](./docs/screenshots/dashboard-preview.png)

*KPIs, evolución mensual, top clientes/proveedores e IVA trimestral — [ver dashboard completo](./docs/screenshots/dashboard-full.png)*

### Subida de facturas

![Flujo de subida de facturas](./docs/screenshots/upload.png)

*Sube una factura PDF; la IA extrae los datos fiscales estructurados al dashboard.*
...
- **Experiencia de producto** — flujo de usuario claro (registro → subida → revisión/confirmación → dashboard).
...
```mermaid
flowchart LR
  Usuario[Usuario Web] -->|Sube PDF| Frontend[Frontend React + Vite]
  Frontend -->|HTTP / JSON / JWT| Backend[Backend Express API]
  Backend -->|Lee/Escribe| SQLite[(SQLite)]
  Backend -->|Extrae texto PDF| PdfParse[pdfjs-dist]
  PdfParse -->|Texto factura| Backend
  Backend -->|Una llamada chat.completions, solo JSON| Azure[Azure AI Foundry GPT-4o]
  Azure -->|JSON campos fiscales| Backend
  Backend -->|Respuestas conforme al contrato| Frontend
```
...
Hemos elegido deliberadamente un **pipeline de extracción de una sola llamada** en lugar de una arquitectura multi-agente. Para datos estructurados de facturas, una única llamada bien diseñada que devuelve JSON validado es más fiable, más barata, más fácil de depurar y más rápida que orquestar varios agentes — y evita los modos de fallo que introducen los sistemas multi-agente cuando lo que realmente se necesita es una respuesta determinista.
...
Los **nombres de campos del contrato** están en español por vocabulario fiscal (`numero`, `fecha`, `base_imponible`, `iva_*`, `irpf_*`, `tipo`, etc.) y **no deben traducirse** (están alineados con formularios y terminología en España). Traducirlos rompería el mapeo a documentos oficiales.
